...
void rgba2bgra(uint8_t *pixels, int x, int y, int bpp) {
    for (int i = 0; i < y * 4; i++) {
        for (int j = 0; j < x; j+=4) {
...
int isfarbfeld(FILE *fp) {
    char buff[9];
    fread(buff, sizeof(uint8_t), 8, fp);
    fseek(fp, 0, SEEK_SET);
    buff[8] = 0;
...
#define ERROR { fprintf(stderr, "%s\n", strerror(errno)); return errno; }
    ...
    isff = isfarbfeld(image);
    if (isff) {
        pixels = decode_farbfeld(image, &img_w, &img_h);
        if (!pixels) return 1;
    } else {
        pixels = stbi_load_from_file(image, &img_w, &img_h, &img_bpp, STBI_rgb_alpha);
        if (!pixels) return 1;
        rgba2bgra(pixels, img_w, img_h, img_bpp);
    }
